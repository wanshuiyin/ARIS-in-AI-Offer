...
- If $A_t > 0$ (this action beats baseline), we want to increase $\pi_\theta(a_t|s_t)$, but once $r_t$ exceeds $1+\epsilon$, the clip term zeroes out the gradient of the objective for that (state, action) pair — the objective no longer has any incentive to keep pushing it up. This is not a hard constraint/projection on the value of $r_t$: in practice $r_t$ can still cross $1+\epsilon$ because of a large single-step gradient or a later epoch's update (which is exactly why we monitor `clip_frac`).
...
**Degeneration of GAE in LLMs**: in RLHF typically $\gamma=1$, and the KL penalty is applied per token (see the $\tilde r_t$ definition in §2.4), so the reward for intermediate tokens is not 0 — it is $\tilde r_t=-\beta\log\frac{\pi_\theta(y_t|\cdot)}{\pi_\text{ref}(y_t|\cdot)}$. Accordingly $\delta_t=\tilde r_t+V(s_{t+1})-V(s_t)$ for intermediate tokens, and $\delta_T=R_T-\beta\log\frac{\pi_\theta(y_T|\cdot)}{\pi_\text{ref}(y_T|\cdot)}+V(s_{T+1})-V(s_T)$ for the terminal token. In this case GAE is equivalent to "value baseline + reward backflow."
...
Within the entire response, **all tokens share the same $\hat{A}_i$** (this is the outcome-supervision version of GRPO — also the mainstream usage in R1/DeepSeekMath — since there is no value model providing per-token baseline, only sequence-level reward and group-relative advantage). The original DeepSeekMath paper also defines a process-supervision variant: each step has its own reward, and the advantage of token $t$ is $\hat{A}_{i,t}=\sum_{\text{index}(j)\ge t}\tilde{r}_i^{\text{index}(j)}$, which decreases with $t$ and is not shared across the whole response.
...
2. **R1**: add a small SFT cold-start + multi-stage RL (reasoning RL → SFT → general RL). R1 itself is a 671B-parameter MoE model built on the DeepSeek-V3 architecture (about 37B parameters activated per token); the open-source **DeepSeek-R1-Distill** 32B/70B (dense models SFT-distilled from bases like Qwen2.5-32B and Llama-3.3-70B on data generated by R1) compete with or partially surpass o1-mini on math reasoning.
...
After DPO, 2024–2025 saw a surge of improvements, and interviews frequently ask "what's the difference between X and Y." The table below compares **core loss differences**; notation is unified as: $\pi$ = current policy, $\pi_\text{ref}$ = SFT model, $\beta$ = temperature parameter / KL regularization coefficient (in $\exp(r/\beta)$, larger $\beta$ pulls the distribution closer to $\pi_\text{ref}$, analogous to temperature $T$ in a Boltzmann distribution; strictly speaking it is $1/\beta$ that matches the physics convention of "inverse temperature" multiplied directly onto the energy/reward), $r$ = implicit reward.
...
where $\gamma$ is the target reward margin (encourages $r_w - r_l$ to exceed $\gamma$ by as much as possible; since the loss is $-\log\sigma(\cdot)$ — a softplus form, not a hinge — the gradient past the margin only decays exponentially toward 0, rather than being hard-clipped to 0 like a hinge loss).

> ✅ **SimPO advantages** —

- **No $\pi_\text{ref}$ needed during training**, saves a copy of model weights in memory.
- **Length-normalization** mitigates DPO's length bias (DPO favors long answers: DPO uses the log-ratio $\log\pi_\theta(y)-\log\pi_\text{ref}(y)$, summed over all tokens in the response, so if the per-token log-ratio averages positive, a longer $y$ accumulates a larger total log-ratio in the sum — not because its raw log-prob is itself more negative — systematically getting a larger implicit reward margin. This is exactly why SimPO introduces length-normalization, dividing by $|y|$).
...
Both **drop the value model**, in the same lineage as GRPO — fundamentally all using **multi-sample or greedy baselines to replace the critic**; the difference is not limited to baseline form (see below).
...
- The three share a philosophy: **bypass the value model, use sample baselines**; but the difference is not limited to baseline form: GRPO additionally does std normalization (z-score — dividing by the standard deviation in addition to subtracting the mean), whereas RLOO/ReMax typically only subtract the mean; GRPO uses PPO-clip (importance ratio + clipping, allowing multiple epochs over the same rollout batch), whereas RLOO/ReMax are pure REINFORCE (no importance-ratio clipping, usually a single on-policy update). The three only share the trait of not using a value model/critic, relying instead on a sampled baseline.
...
6. **Composite reward**: weighted combination of rule-based + RM-based; the rule part is usually harder to hack than a neural RM (but not absolutely — checkers/unit tests with incomplete coverage or bugs can still be exploited, e.g. answer-format matching, unhandled edge cases; see §5.3).
...
**DPO with same base**: only needs $\pi_\theta$ + $\pi_\text{ref}$ = 112 + 14 = **126 GB** (about half of PPO's ~252GB).

**GRPO with same base** (neural RM): $\pi_\theta$ + $\pi_\text{ref}$ + RM = 112 + 14 + 14 = **140 GB** (saves one value copy).
...
- IPO uses squared loss + fixed margin $1/(2\beta)$: pulls the log-ratio difference of each preference pair toward a fixed, finite target $1/(2\beta)$, avoiding DPO's sigmoid loss being pushed to $+\infty$ under deterministic preferences (implicit reward margin diverging) — this prevents margin divergence, not a strict global boundedness guarantee on the reward function itself
...
- **Reward margin** $\gamma$: require $r_w - r_l \ge \gamma$; past that, loss/gradient asymptotically decays toward 0 (softplus form, not a hinge hard cutoff)
- Saves about 11% memory relative to DPO (drops the frozen $\pi_\text{ref}$ weights, 14GB, going from 126→112 GB); but losing the KL anchor requires more careful tuning of $\beta, \gamma$
...
Pitfalls: writing per-token advantage (wrong under the outcome-supervision version; the process-supervision version is genuinely per-token); or not knowing the commonality of GRPO/RLOO/ReMax.
...
- Adding on the loss can still be per-token (e.g. GRPO computes the K3 estimator per token, see §5.2) — it does not inherently lose token-level resolution; the real difference is that KL added to the reward propagates through GAE/return recursively into every token's advantage, whereas KL added to the loss is an additive penalty term independent of the return, not participating in value bootstrapping
...
- **Dynamic sampling (DAPO)**: filter out prompts whose group rewards are all identical (all-correct or all-wrong, std=0, advantage always 0 → zero gradient), and keep resampling new prompts to keep the number of samples with useful gradient constant within the batch; the per-prompt sample count $G$ itself is usually kept fixed, not adaptively adjusted by reward variance
