...
4. **LLaVA = projector + 2-stage train**: Liu et al. 2023 (NeurIPS) use a **single linear projection matrix $W$** to project frozen CLIP visual features into LLM token space (**the 2-layer MLP projector is the LLaVA-1.5 upgrade**). **Stage 1** trains only the projector for feature alignment (caption data); **Stage 2** unfreezes the LLM for visual instruction tuning (158K instructions generated by GPT-4).
...
> 💡 **2024–2025 trend** — the SigLIP family has stably surpassed CLIP on zero-shot ImageNet and downstream retrieval; representative open-weight VLMs using SigLIP-So400M are **PaliGemma / LLaVA-OneVision**. **Molmo/PixMo's main models still use OpenAI CLIP ViT-L/14@336 (SigLIP appears only in their paper's ablation); the InternVL series uses their in-house InternViT; Qwen2-VL trains its own ViT; LLaVA-1.5/1.6 still use CLIP ViT-L/14** — "switching to SigLIP" is not an industry consensus.
...
Image ──► CLIP ViT-L/14 ──► visual features  z_v ∈ R^{N × d_v}
                                  │
                                  │  W ∈ R^{d_v × d_LLM}   ← projector (1.0 single linear / 1.5 2-layer MLP)
                                  ↓
                            H_v ∈ R^{N × d_LLM}
                                  │
...
- **Loss**: LLaVA uses next-token prediction (the LLM takes visual features as a prefix to generate a caption); **BLIP-2 does not connect an LLM here** — the Q-Former's Stage 1 is representation learning, jointly optimizing ITC + ITM + ITG over a frozen image encoder, and the frozen LLM is only attached at Stage 2 (generative learning)
...
[CLIP] N=8, D=512, init logit_scale=ln(1/0.07)→1/τ≈14.285: loss ≈ 2.25 (slightly above log(N)=2.08; loss≈log N holds only when 1/τ is very small)  ✓
