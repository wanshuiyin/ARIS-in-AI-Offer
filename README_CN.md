...
- **2026-06-16** — ![FIX](https://img.shields.io/badge/FIX-d73a49?style=flat-square) 🩹 **全集 LaTeX `<` 渲染修复**（[#18](https://github.com/wanshuiyin/ARIS-in-AI-Offer/issues/18) by [@wangzh1998](https://github.com/wangzh1998)）—— [`tools/render_html.py`](tools/render_html.py) 现在对数学 passthrough 里的 `<`/`>`/`&` 做 HTML 转义，含 `<` 的 LaTeX（如 `y_{<t}`）不再被浏览器当标签吃掉。全部 51 篇 tutorial 重渲，`--reproduce` gate 绿。
- **2026-06-15** — ![QUALITY](https://img.shields.io/badge/QUALITY-2ea44f?style=flat-square) 🔬 **#24/#25 上线后修正** —— 修了 LoRA DoRA magnitude 轴（逐**输出** `dim=1`，对齐 HF PEFT，非逐输入）、两处 RAG 术语（HyDE → 假设**文档**；CRAG 打分对象是**已检索文档**相关性），并补了紧凑的 2026 前沿（S-LoRA · *LoRA Learns Less and Forgets Less* · QDoRA · SPLADE · ColBERTv2/PLAID · Contextual Retrieval · BEIR/MTEB）。中英双语同步；引用已核。
