...
- **2026-07-13** — ![QUALITY](https://img.shields.io/badge/QUALITY-2ea44f?style=flat-square) 🔍 **扩散生成簇重扫收尾 —— 8 篇教程、66 处修复，全集 28 篇正式全部纳入 GPT-5.6-sol** —— 上一轮重扫（07-12）暂缓的扩散/生成媒体簇——`3d_generation`、`diffusion_distillation`、`diffusion_foundations`、`diffusion_post_training`、`flow_matching`、`image_generation_systems`、`vae_vqvae_vqgan`、`video_generation`——补齐同一套两阶段流水线：67 条候选 → 55 CONFIRMED + 11 NEEDS_NUANCE 落地，2 条 REFUTED 保留原文。P0 密度明显高于上一批——扩散模型数学的隐蔽失效模式更多，最高频的一类是把"conditional path 是直线"和"训练后 marginal ODE 轨迹是直线"混为一谈（这正是 Reflow 存在的意义），在 `diffusion_foundations` / `flow_matching` 反复出现并逐处修正。修了 5 处**真实代码 bug**（iCT 常数漏 `√D` 缩放、iCT 用随机排序代替确定性 Karras 网格、FSQ 缺偶数 level 的 parity offset、LFQ entropy 正则方向写反、DDPO 伪代码对确定性 terminal transition 除零）和大量机制误解（ADD 蒸馏项反复被误写成像素 MSE、DMD2 被推荐成"GAN-free"实为其核心恰是加了 GAN loss、SD3 时间方向说反、SD/SDXL middle block 被误判"无 cross-attention"等）。另清理 3 处跨文件一致性遗留（SD3-Turbo 被误列为 Reflow 例子，实为 LADD；OpenMagViT2 被误列为 FSQ 例子，实为 LFQ）。全部 8 篇 ZH+EN 同步、重渲染、sidecar 留痕。Gate：`tools/verify_reviews.py --mode strict --reproduce` → 59 OK / 0 FAIL / PASS。（[#32](https://github.com/wanshuiyin/ARIS-in-AI-Offer/pull/32) · [bfae8f1](https://github.com/wanshuiyin/ARIS-in-AI-Offer/commit/bfae8f1)）
