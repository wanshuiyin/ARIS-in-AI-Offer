...
**📚 Jump to a topic** — 25 first-party cheat sheets across 7 categories + 1 community-contributed category:
...
- **2026-06-16** — ![FIX](https://img.shields.io/badge/FIX-d73a49?style=flat-square) 🩹 **LaTeX `<` rendering fixed collection-wide** ([#18](https://github.com/wanshuiyin/ARIS-in-AI-Offer/issues/18) by [@wangzh1998](https://github.com/wangzh1998)) — [`tools/render_html.py`](tools/render_html.py) now HTML-escapes `<`/`>`/`&` inside math passthrough, so LaTeX like `y_{<t}` is no longer parsed as an HTML tag (it ate the rest of the formula). All 51 tutorial pages re-rendered; `--reproduce` gate green.
- **2026-06-15** — ![QUALITY](https://img.shields.io/badge/QUALITY-2ea44f?style=flat-square) 🔬 **Post-ship corrections to #24/#25** — fixed the LoRA DoRA magnitude axis (per-**output** `dim=1`, matching HF PEFT, not per-input), two RAG terms (HyDE → hypothetical **document**; CRAG scores **retrieved-doc** relevance), and added compact 2026-frontier notes (S-LoRA · *LoRA Learns Less and Forgets Less* · QDoRA · SPLADE · ColBERTv2/PLAID · Contextual Retrieval · BEIR/MTEB). Both editions; citations verified.
