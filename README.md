...
- **2026-07-13** — ![QUALITY](https://img.shields.io/badge/QUALITY-2ea44f?style=flat-square) 🔍 **Diffusion-cluster resweep completes the sweep — 8 tutorials, 66 fixes, all 28 tutorials now under GPT-5.6-sol** — the diffusion/generative-media cluster deferred from the prior resweep (07-12) — `3d_generation`, `diffusion_distillation`, `diffusion_foundations`, `diffusion_post_training`, `flow_matching`, `image_generation_systems`, `vae_vqvae_vqgan`, `video_generation` — went through the same two-stage pipeline: 67 candidate findings → 55 CONFIRMED + 11 NEEDS_NUANCE fixed, 2 REFUTED correctly left untouched. Notably higher P0 density than the first batch — diffusion-model math has more subtle failure modes. The single most-recurring error: conflating "the conditional path is a straight line" with "the marginal ODE trajectory the model learns is a straight line" (they're not the same — this is exactly why Reflow exists), which recurred across `diffusion_foundations` and `flow_matching` and was fixed at every occurrence. Fixed 5 **real code bugs** (iCT's Pseudo-Huber constant missing the paper's `√D` scaling, iCT's schedule using sampled-then-sorted lognormal σ instead of a deterministic Karras/EDM grid, FSQ's bound function missing the parity-offset shift for even levels, LFQ's entropy-regularization loss pointed the wrong direction, DDPO's pseudocode dividing by a deterministic zero-variance terminal transition) plus a long list of mechanism-level misconceptions (ADD's distillation loss repeatedly mislabeled as pixel MSE, DMD2 recommended as "GAN-free" when its core change is *adding* a GAN loss, SD3's time direction stated backwards, SD/SDXL's middle block said to lack cross-attention when it has it, and more). Also swept up 3 cross-file consistency flags (SD3-Turbo mislabeled as a Reflow example when it's LADD; OpenMagViT2 cited as an FSQ example when it's LFQ). All 8 files' ZH+EN editions fixed in parallel, re-rendered, sidecars updated. Gate: `tools/verify_reviews.py --mode strict --reproduce` → 59 OK / 0 FAIL / PASS. ([#32](https://github.com/wanshuiyin/ARIS-in-AI-Offer/pull/32) · [bfae8f1](https://github.com/wanshuiyin/ARIS-in-AI-Offer/commit/bfae8f1))
