...
EXEMPTIONS = {
    "docs/index.html":
        "the site hub (one page listing every tutorial), generated by tools/build_index.py "
        "from the catalogue in that file — not a render_html.py product, so it carries no "
        "aris:source meta. Rebuild with `python3 tools/build_index.py`; reviewed at the "
        "artifact boundary like the blogs (design + code round recorded in docs/index.review.json).",
...
def classify(html_path: Path, root: Path) -> Record:
    ...
    if not src_rel:
        ...
        if rel_html.startswith("docs/code/"):
            return Record(html=rel_html, status=EXEMPT,
                          detail="exempt — source-code viewer page generated by tools/build_code_pages.py "
                                 "from docs/tutorials/code/*.py; rebuild with `python3 tools/build_code_pages.py`")
